...
GO
/****** Object:  UserDefinedFunction [dbo].[FN_GET_COMPANY_DECIMAL_SETTING]    Script Date: 19-07-2022 14:55:47 ******/
SET ANSI_NULLS ON
GO
...
(
      @ROUNDING_VALUE NVARCHAR(500),
      @ROUNDING_PARAM NVARCHAR(50)
)
RETURNS FLOAT
AS      
BEGIN
		DECLARE @SQL_QUERY NUMERIC(20,9)
		...
		BEGIN				
			SET @SQL_QUERY =
			(SELECT 
			(CASE WHEN ABS(CONVERT(NUMERIC(20,9), @ROUNDING_VALUE) - ROUND(CONVERT(NUMERIC(20,9), @ROUNDING_VALUE), @ROUNDING_PLACE_VALUE, 1)) * POWER(10, @ROUNDING_PLACE_VALUE + 1) = 6 
				THEN 
					ROUND(CONVERT(NUMERIC(20,9), @ROUNDING_VALUE), @ROUNDING_PLACE_VALUE, 
						CASE WHEN CONVERT(NUMERIC(20,9), ROUND(ABS(CONVERT(NUMERIC(20,9), @ROUNDING_VALUE)) * POWER(10, @ROUNDING_PLACE_VALUE), 0, 1)) % 2 = 1 
						THEN 
							0 
						ELSE 
							1 
		...
		BEGIN			
			SET @SQL_QUERY = 
			 (	
				--CASE WHEN ABS(CONVERT(NUMERIC(20,9),@ROUNDING_VALUE) - ROUND(CONVERT(NUMERIC(20,9),@ROUNDING_VALUE), @ROUNDING_PLACE_VALUE, 1)) * POWER(10, @ROUNDING_PLACE_VALUE+1) = 6 
				--	THEN CEILING(CONVERT(NUMERIC(20,9),@ROUNDING_VALUE) * Power(10,@ROUNDING_PLACE_VALUE))/POWER(10, @ROUNDING_PLACE_VALUE)
				--   ELSE ROUND(CONVERT(NUMERIC(20,9),@ROUNDING_VALUE), @ROUNDING_PLACE_VALUE)
				--END
				CEILING(CONVERT(NUMERIC(20,9),@ROUNDING_VALUE) * Power(10,@ROUNDING_PLACE_VALUE))/POWER(10, @ROUNDING_PLACE_VALUE)
			 )
		END
		...
		BEGIN	
			...
			(		
				--CASE WHEN ABS(CONVERT(NUMERIC(20,9),@ROUNDING_VALUE)- ROUND(CONVERT(NUMERIC(20,9),@ROUNDING_VALUE), @ROUNDING_PLACE_VALUE, 1)) * POWER(10, @ROUNDING_PLACE_VALUE+1) = 6 
				--		THEN FLOOR(CONVERT(NUMERIC(20,9),@ROUNDING_VALUE) * POWER(10,@ROUNDING_PLACE_VALUE))/POWER(10, @ROUNDING_PLACE_VALUE)
				--	ELSE ROUND( @ROUNDING_VALUE, @ROUNDING_PLACE_VALUE)
				--END
				FLOOR(CONVERT(NUMERIC(20,9),@ROUNDING_VALUE) * POWER(10,@ROUNDING_PLACE_VALUE))/POWER(10, @ROUNDING_PLACE_VALUE)
			)
		END
